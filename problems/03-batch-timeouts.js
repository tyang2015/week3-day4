...
function batchTimeouts(callbacks, delays) {
  ...
  if (callbacks.length <= 0) return timeoutObjs;

  else {
        let timeObj = setTimeout(callbacks[0], delays[0]);
        
        timeoutObjs.push(timeObj);
  }
    return batchTimeouts(callbacks.shift(), delays.shift())
}
// function batchTimeouts(callbacks, delays) {
...
//     let time = setTimeout(callback, delays[i]);
    
//     console.log(callback)
    
//     timeoutObjs.push(time)
